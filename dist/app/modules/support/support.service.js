...
const createSupport = async (user, payload) => {
    ...
    const data = { ...payload, userId: user === null || user === void 0 ? void 0 : user.authId, status: support_1.SUPPORT_STATUS.IN_PROGRESS };
    try {
...
const getSingleSupport = async (id) => {
    ...
    const result = await support_model_1.Support.findById(id).populate('userId');
    if (!result) {
